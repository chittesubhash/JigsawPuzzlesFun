...
@implementation SlidingMenuColors
...
@synthesize isFromColoring;
@synthesize journalDelegateObj;
@synthesize isFromMenuSliderPage;


-(id) initWithArray:(NSMutableArray*)items cols:(int)cols rows:(int)rows position:(CGPoint)pos padding:(CGPoint)pad verticalPaging:(bool)vertical isFromSliderPage:(BOOL)isSliderPage
{
	if ((self = [super init]))
	{
		...
		iCurrentPage = 0;
		bMoving = false;
		bSwipeOnlyOnMenu = YES;
		menuOrigin = pos;
		fMoveDeadZone = 10;
		bVerticalPaging = vertical;
		fAnimSpeed = 1;
        isFromMenuSliderPage=isSliderPage;
...
-(void) buildGrid:(int)cols rows:(int)rows
{
    ...
    if (isFromMenuSliderPage) {
        if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad) {
            winSize=CGSizeMake(710, 768);
        }
        else
        {
            winSize=CGSizeMake(710, 320);
...
//    [journalDelegateObj displayScrollViewIndicator:iCurrentPage];
